...
#生成低通和高通滤波器
hpf = [np.zeros((h,w,3)) for _ in range(0,10)]
R = (h+w)//5  #或其他
p = 5
# for x in range(w):
#     for y in range(h):
...
for i in range(0,10):
    delta = (w-2*p)//20
    for x in range(w//2-i*delta,w//2+i*delta):
        for y in range(p,h-p):
            hpf[i][y,x,:] = 1
lpf = [1-hpf[_] for _ in range(0,10)]

for i in range(0,10):
    os.makedirs(output_path+f"/{i}",exist_ok=True)
    for j in num_list:
        ...
        cv2.imwrite(output_path+f'/{i}/images/LPF{j}.jpg',img_l)
        #生成高频分量图
