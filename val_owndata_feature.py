...
class MVSSystem(LightningModule):
    ...
    def init_volume(self):
        ...
        # self.ori_volume = RefVolume(volume_feature.detach()).to(device)
        # ckpt2 = torch.load("./runs_fine_tuning/slight9/ckpts/volume.tar")
        # ckpt2 = torch.load("./runs_fine_tuning/all_size8/ckpts/volume.tar")
        # self.ori_volume.load_state_dict(ckpt["volume_0"])
        # print(self.ori_volume.feat_volume.shape)

        # self.volume = RefVolume(volume_feature.detach()).to(device) 
        # self.volume = RefVolume(0.5*volume_feature.detach()+ 0.5*self.ori_volume.feat_volume.detach()).to(device) 

        # self.volume = RefVolume(volume_feature.detach()).to(device)
        # self.volume.load_state_dict(ckpt2["volume_0"]) 
        # print(volume_feature)
        # print(self.volume.feat_volume)
        # torch.save(self.volume.feat_volume,"./feature/slight_volume0.pth")
        ori_volume = torch.load("./feature/all_volume3.pth")
        with torch.no_grad():
            volume_feature[:,:,:,70:140,100:160] = ori_volume[:,:,:,70:140,100:160]
        # self.volume = RefVolume(0*volume_feature.detach()+1*ori_volume.detach()).to(device) 
        self.volume = RefVolume(volume_feature.detach()).to(device) 
        # img1 = transforms.ToPILImage()(volume_feature[0][0][0])
        # img1.save(f"./feature/new1.png")
        # img2 = transforms.ToPILImage()(ori_volume[0][0][0])
        # img2.save(f"./feature/ori1.png")
        del volume_feature
