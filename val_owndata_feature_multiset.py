...
class MVSSystem(LightningModule):
    def __init__(self, args):
        ...
        for i in range(self.args.multiset_num):
            self.train_dataset.append(dataset(args, split='train', idx=i))
        for i in range(self.args.multiset_num):
    ...
    def init_volume(self):
        ...
        # for i in range(self.args.multiset_num):
        #     self.volume = RefVolume(volume_feature[0].detach()).to(device)
        #     self.volume.load_state_dict(ckpt2[f"volume_{i}"]) 
        #     torch.save(self.volume.feat_volume,f"./feature/all_volume{i}.pth")
        #     img1 = transforms.ToPILImage()(self.volume.feat_volume[0][0][0])
        #     img1.save(f"./feature/test{i}.png")
        self.volume = []
        for i in range(self.args.multiset_num):
            tmp_volume = torch.load(f"./feature/all_volume{i}.pth")
            self.volume.append(RefVolume(1*volume_feature[i].detach() + 0*tmp_volume).to(device)) 
    ...
    def validation_step(self, batch, batch_nb):
        ...
        with torch.no_grad():
            ...
            os.makedirs(f'runs_fine_tuning/{self.args.expname}/valid_set/',exist_ok=True)

            img_vis = torch.cat((img,rgbs,img_err_abs*10,depth_r.permute(1,2,0)),dim=1).numpy()
            imageio.imwrite(f'runs_fine_tuning/{self.args.expname}/valid_set/{self.current_epoch}_{self.global_step:08d}_{self.idx:02d}.png', (img_vis*255).astype('uint8'))
