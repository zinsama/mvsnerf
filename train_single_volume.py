...
class MVSSystem(LightningModule):
    def __init__(self, args):
        ...
        path = f'{save_dir}/volume_init.tar'
        ckpt = {}
        ckpt[f'volume'] = self.volume.state_dict()
    ...
    def init_volume(self):
        ...
        if args.ckpt:
            ...
            if 'volume' not in ckpts.keys():
                self.MVSNet.train()
                with torch.no_grad():
                    volume_feature, _, _ = self.MVSNet(self.imgs[0], self.proj_mats, self.near_far_source, pad=args.pad, lindisp=args.use_disp)
            else:
    ...
    def training_step(self, batch, batch_nb):
        ...
        rgbs, disp, acc, depth_pred, alpha, extras = rendering(args, self.pose_source, xyz_coarse_sampled, xyz_NDC, z_vals, rays_o, rays_d,
                                                       self.volume, self.imgs[idx],  **self.render_kwargs_train)
    ...
    def save_ckpt(self, name='latest'):
        ...
        ckpt2 = {}
        for i in range(self.args.multiset_num):
            ckpt2[f'volume_{i}'] = self.volume.state_dict()
