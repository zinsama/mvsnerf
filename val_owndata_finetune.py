...
class MVSSystem(LightningModule):
    ...
    def training_step(self, batch, batch_nb):
        ...
        # if self.global_step == 1 or self.global_step == 5999:
        #     # self.save_ckpt(f'{self.global_step}')
        #     rgbs_target = rgbs_target.detach().cpu()
        #     rgbs =  rgbs.detach().cpu()
        #     print(rgbs.shape,rgbs_target,H,W,"!!!!!!!!!")
        #     rgbs = torch.clamp(rgbs.reshape(H, W, 3),0,1)
        #     img_vis = torch.cat((rgbs_target,rgbs),dim=1).numpy()
        #     imageio.imwrite(f'runs_fine_tuning/{self.args.expname}/{self.args.expname}/train_{self.global_step:08d}_{self.idx:02d}.png', (img_vis*255).astype('uint8'))
        return  {'loss':loss}
    ...
    def validation_step(self, batch, batch_nb):
        ...
        with torch.no_grad():
            ...
            if self.args.savedir is not None:
                ...
                os.makedirs(f'runs_fine_tuning/{self.args.expname}/{self.args.savedir}/',exist_ok=True)

                img_vis = torch.cat((img,rgbs,img_err_abs*10,depth_r.permute(1,2,0)),dim=1).numpy()
                imageio.imwrite(f'runs_fine_tuning/{self.args.expname}/{self.args.savedir}/{self.global_step:08d}_{self.idx:02d}.png', (img_vis*255).astype('uint8'))
